	...
	proc/EquipRank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0)
		...
		if(H.mind)
			...
			switch(rank)
				...
				if("Statthalter")
					...
					captain_announcement.Announce("All hands, Statthalter [H.real_name] on deck!", new_sound=announce_sound)
		...
		if(job.has_email)
			...
			if(H.char_branch && H.char_branch.email_domain)
				...
				domain = "freemail.nt"
			var/sanitized_name = sanitize(replacetext(replacetext(rlowertext(H.real_name), " ", "."), "'", ""))
		...
		if(H.religion && H.mob_ideology)//In case they somehow don't have one.
			H.mind.religion = H.religion
			H.mind.mind_ideology = H.mob_ideology
			H.mob_ideology = ideologies[H.mob_ideology.ideology_flag]
			H.mind.mind_ideology = ideologies[H.mob_ideology.ideology_flag]
			H.mob_ideology.followers.Add(H)
			H.verbs += /mob/living/carbon/human/proc/start_election
			/*to_chat(H, "Помните! Не вступайте в контакт с приверженцами <b>[IDEOLOGY_COMMUNISM]а</b>, <b>[IDEOLOGY_FASCISM]а</b>. И сторонниками такой мысли, как <b>[IDEOLOGY_LIBERT]</b>.")
			if(prob(90))//Only a 5% chance to not remember the prayer.
				H.mind.kaiser_edict = accepted_kaiser
				to_chat(H, "<span class='notice'>Эдикт Кайзера следующий: <b>[H.mind.kaiser_edict]</b> Запоминайте и повинуйтесь.</span>")
			else
				to_chat(H, "<span class='danger'>Вы не помните эдикт Кайзера... Это достаточно плохо, потому что это его последний закон.")
			if(H.ideology_is_fasci())
				to_chat(H, "Вы приверженец <b><font color='red'>ФАШИЗМА</font>. Де&#255;тельность таких партий незаконна. Но всегда можно завоевать власть, ведь так?")
				if(prob(50))
					H.mind.fasci_edict = accepted_fasci
					to_chat(H, "<span class='notice'>Сообщение вашего Лидера: <b>[H.mind.fasci_edict]</b>.</span>")
				else
					to_chat(H, "<span class='danger'>Хоть вы и фашист, но Лидера так и не знаете. Может быть лучше стать самим Лидером? Или найти его?")
			else if(H.ideology_is_commie())
				to_chat(H, "Вы приверженец <b><font color='red'>КОММУНИЗМА</font>. Коммунистические партии запрещены. Действуйте подпольно и установите диктатуру пролетариата.")
				if(prob(50))
					H.mind.commie_edict = accepted_commie
					to_chat(H, "<span class='notice'>Сообщение от ЦК Компартии: <b>[H.mind.commie_edict]</b></span>")
				else
					to_chat(H, "<span class='danger'>Вы не помните сообщение от ЦК Компартии. Но, возможно, вы не член компартии.")
			else if(H.ideology_is_market())
				to_chat(H, "Вы приверженец <b><font color='red'>ЛИБЕРТАРИАНСТВА</font>. Анархо-капиталистические, либертарианские движени&#255; запрещены. Берегитесь <b>Абвера</b>, но можете попытатьс&#255; установить буржуазную диктатуру.")
				if(prob(50))
					H.mind.market_edict = accepted_market
					to_chat(H, "<span class='notice'>Свежа&#255; сводка рынков: <b>[H.mind.market_edict]</b></span>")
				else
					to_chat(H, "<span class='danger'>Сводка рынков прошла как-то мимо вас. Какой-то вы херовый либертарианец, что не следите за рынком ценных бумаг.")*/

...
/mob/living/carbon/human/proc/handle_excrement()
	...
	if(bowels >= 250)
		switch(bowels)
			if(250 to 400)
				if(prob(5))
					to_chat(src, "<b>Нужно идти в туалет.</b>")
					bowels += 15
			if(400 to 450)
				if(prob(5))
			...
			if(500 to 550)
				if(prob(15))
					...
					to_chat(src, "<span class='danger'>ГОСПОДИ, КАК Я ХОЧУ СРАТЬ!!!</span>")
					bowels += 35
			if(550 to INFINITY)
				handle_shit()
		...
		switch(bladder)
			...
			if(400 to 500)
				if(prob(2))
					...
					to_chat(src, "<span class='danger'>Еще чуть-чуть и можно говорить мочевому пузырю 'Ауф Видерзейн'!</span>")
					bladder += 25
			if(500 to 550)
				if(prob(15))
					handle_piss()
				else if(prob(30))
					to_chat(src, "<span class='danger'>ГОСПОДИ, КАК Я ХОЧУ ССАТЬ!</span>")
					bladder += 35
			if(550 to INFINITY)
				handle_piss()
...
/mob/living/carbon/human/proc/handle_shit()
	...
	if (src.bowels >= 30)
		...
		//Poo on the face.
			...
			message = "<span class='danger'><b>[src]</b> срет точно <b>[M]</b> в лицо!</span>"
			M.reagents.add_reagent("poo", 10)

		//Poo on the floor.
		else
			message = "<B>[src]</B> [pick("срет", "какает", "отправл&#255;ет бомбу на пол")]."
			var/obj/item/weapon/reagent_containers/food/snacks/poo/V = new/obj/item/weapon/reagent_containers/food/snacks/poo(src.loc)
			if(reagents)
				reagents.trans_to(V, rand(1,5))
...
/mob/living/carbon/human/proc/handle_piss()
	...
	if((U || S) && gender != FEMALE)//In the urinal or sink.
		...
		if(RC.is_open_container())
			...
			message = "<B>[src]</B> ссыт точно в [RC]."
			var/amount = rand(1,8)
